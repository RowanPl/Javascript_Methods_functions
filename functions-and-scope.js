...
const hoeveelheid = grades.length;
let totaal = 0;
for (let i = 0; i < grades.length; i++){
    totaal = totaal + grades[i];
}
console.log(totaal / hoeveelheid);
...
// Log het antwoord in de terminal.
function averageGrade(ArrayList){
    const hoeveel = ArrayList.length;
    let totaal = 0 ;

    for (let i = 0; i < ArrayList.length; i++){
       totaal = totaal + ArrayList[i];
    }
    const average = totaal / hoeveel ;
    return average;
}
...
function highestGrade(array) {
    if (toString.call(array) !== "[object Array]")
        return "Dit is geen array, Zet ze even tussen blokhaken!";
